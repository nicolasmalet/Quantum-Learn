...
class DataSignal:
    ...
    def prepare_data(self, batch_size: int) -> None:
        ...
        self.X_train = np.reshape(raw_X_train, (-1, self.nb_points_per_period * self.batch_size))
        self.Y_train = np.reshape(raw_Y_train, (-1, 2, self.batch_size))
...
def create_data_sinus_vs_square(nb_periods_train, nb_periods_test) -> DataSignal:
    ...
    choices = np.random.binomial(1, 0.5, [1, nb_periods_train])
    Y_train = np.vstack((choices, 1 - choices))

    for i in range(nb_periods_train):
        X_train[:, i] = sinus if Y_train[0][i] == 1 else square


    X_test = np.zeros([nb_points_per_period, nb_periods_test])
    Y_test = 2 * np.random.binomial(1, 0.5, [1, nb_periods_test]) - 1

    for i in range(nb_periods_test):
        X_test[:, i] = sinus if Y_test[0][i] == 1 else square
