...
class DataCreatorSinus(DataCreator):
    ...
    def __call__(self) -> Data:
        ...
        X_train = np.empty((self.nb_periods_train, NB_POINTS_PER_PERIOD))
        Y_train = np.random.binomial(1, 0.5, (self.nb_periods_train, 1))

        for i in range(self.nb_periods_train):
            X_train[i, :] = sinus if Y_train[i, 0] == 1 else square

        X_test = np.empty((self.nb_periods_test, NB_POINTS_PER_PERIOD))
        Y_test = np.random.binomial(1, 0.5, (self.nb_periods_test, 1))
