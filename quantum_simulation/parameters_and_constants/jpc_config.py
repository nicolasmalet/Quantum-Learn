...
class JPCConfig(Summary):
    ...
    def H_delta(self, delta_a: float | jnp.ndarray, delta_b: float | jnp.ndarray) -> dq.QArray:
        return -delta_a * dq.tensor(self.N_a, dq.eye(self.DIM_B)) - delta_b * dq.tensor(dq.eye(self.DIM_A), self.N_b)

    def H_drive(self, epsilon_a: float | jnp.ndarray, epsilon_b: float | jnp.ndarray) -> dq.QArray:
        H_da = dq.tensor(
    ...
    def Build_H(self, quantum_parameters: QuantumParameters) -> dq.QArray:
        ...
        match self.encoding_observable:
            case 'epsilon':
    ...
    def Build_H_batched(self, quantum_parameters_list: list[QuantumParameters]) -> dq.QArray:
        """
    ...
    def H(self, quantum_parameters: list[QuantumParameters], data: jnp.ndarray, time_interval: jnp.ndarray) -> dq.TimeQArray:

        # 1. Construction du batch H_free ultra-rapide
        H_free_batch = self.Build_H_batched(quantum_parameters)
        ...
        match self.encoding_observable:
            case 'epsilon':
                ...
                values_a = self.f_encoding(epsilon_a, data)
                values_b = self.f_encoding(epsilon_b, data)
                H_encoded = (dq.pwc(time_interval, values_a, self.KAPPA_A * dq.tensor(self.a, dq.eye(self.DIM_B)))
                             + dq.pwc(time_interval, jnp.conj(values_a),
                                      self.KAPPA_A * dq.tensor(self.a_dag, dq.eye(self.DIM_B)))
                             + dq.pwc(time_interval, values_b, self.KAPPA_B * dq.tensor(dq.eye(self.DIM_A), self.b))
                             + dq.pwc(time_interval, jnp.conj(values_b),
                                      self.KAPPA_B * dq.tensor(dq.eye(self.DIM_A), self.b_dag))
                             )
            case 'g_conv':
                g_conv = quantum_parameters[0].g_conv
                values = self.f_encoding(g_conv, data)
            ...
            case 'g_sq':
                ...
                values = self.f_encoding(g_sq, data)
